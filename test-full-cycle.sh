...

{
    ...
    echo ""
    systemctl status streamdeck-pipewire.service --no-pager -l | head -n 5 || echo "PipeWire (streamdeck) status unavailable"
    echo ""
    systemctl status streamdeck-audio-capture.service --no-pager -l | head -n 5 || echo "Audio capture status unavailable"
